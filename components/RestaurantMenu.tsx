...
import { Plus, Utensils, Coffee, ShoppingCart, Check } from 'lucide-react';
...
const RestaurantMenu: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addMenuItem({
        ...formData,
        imageUrl: formData.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=500' // Default placeholder
    });
    setShowModal(false);
        ...
        {filteredItems.map(item => (
            ...
            >
                <div className="h-48 overflow-hidden relative">
                    ...
                    <span className={`absolute top-2 right-2 px-2 py-1 rounded text-xs font-bold text-white ${item.category === 'food' ? 'bg-orange-500' : 'bg-blue-500'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                 <div>
                    ...
                    <input type="number" step="0.01" required className="w-full p-2 border rounded-lg" value={formData.price} onChange={e => setFormData({...formData, price: Number(e.target.value)})} />
                 </div>
                 <div>
              ...
              <div>
                ...
                <input type="url" placeholder="https://..." className="w-full p-2 border rounded-lg" value={formData.imageUrl} onChange={e => setFormData({...formData, imageUrl: e.target.value})} />
              </div>
