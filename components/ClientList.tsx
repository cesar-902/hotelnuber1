...
import { Plus, Search, Trophy, History, CheckCircle, CreditCard, Banknote, QrCode, Utensils, BedDouble, Calendar, User, X, PlusCircle, Filter } from 'lucide-react';
import { Client, Stay, Room, RoomCategory } from '../types';

const ClientList: React.FC = () => {
  const { clients, addClient, searchClients, getClientStays, checkoutStay, rooms, createStay, stays, getAvailableRooms, loyaltyConfig } = useHotel();
  const [activeTab, setActiveTab] = useState<'list' | 'booking' | 'history'>('list');
  const [searchTerm, setSearchTerm] = useState('');
  const [historySearchTerm, setHistorySearchTerm] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [selectedClient, setSelectedClient] = useState<Client | null>(null);
  const [pointsToUse, setPointsToUse] = useState(0);
  const [historySortOrder, setHistorySortOrder] = useState<'date-desc' | 'date-asc' | 'name-asc' | 'name-desc'>('date-desc');
  ...
  // Booking Form State
  const [selectedBookingClient, setSelectedBookingClient] = useState<Client | null>(null);
  ...
  const filteredBookingClients = clientSearchQuery.length > 0 ? searchClients(clientSearchQuery) : [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.phone.length < 14) { // (XX) XXXXX-XXXX is 15 chars, (XX) XXXX-XXXX is 14
  ...
  const handlePayment = (method: string) => {
      ...
      if (result) {
        const message = `Pagamento realizado via ${method}.\n\n` +
          `✅ Checkout concluído com sucesso!\n\n` +
          `💰 Total pago: R$ ${checkoutData.roomRate + checkoutData.totalExtras - result.discount}\n` +
          `${result.discount > 0 ? `🎁 Desconto (${pointsToUse} pontos): -R$ ${result.discount.toFixed(2)}\n` : ''}` +
          `⭐ Pontos ganhos: +${result.earnedPoints}\n` +
          `${result.pointsUsed > 0 ? `📉 Pontos usados: -${result.pointsUsed}\n` : ''}`;
  ...
  const handleBooking = () => {
    if (!selectedBookingClient || selectedRooms.length === 0 || !dates.checkIn || !dates.checkOut) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-4 border-b border-gray-200">
        <button 
          className={`pb-2 px-4 font-medium ${activeTab === 'list' ? 'border-b-2 border-hotel-600 text-hotel-600' : 'text-gray-500'}`}
          onClick={() => setActiveTab('list')}
        >
          Lista de Clientes
        </button>
        <button 
          className={`pb-2 px-4 font-medium ${activeTab === 'booking' ? 'border-b-2 border-hotel-600 text-hotel-600' : 'text-gray-500'}`}
            ...
            <div className="relative">
              ...
              {!selectedBookingClient ? (
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <div>
                          <p className="font-bold text-sm text-blue-900">{selectedBookingClient.name}</p>
                          <p className="text-xs text-blue-700">CPF: {selectedBookingClient.document}</p>
                      </div>
                      <button onClick={() => setSelectedBookingClient(null)} className="text-blue-500 hover:text-blue-700">
                          <X size={18} />
                      </button>
                  </div>
              )}
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {availableRooms && availableRooms.length > 0 && (
                ...
                <div className="sticky bottom-4 bg-white p-4 shadow-lg border border-gray-100 rounded-xl flex justify-between items-center mt-6">
                    ...
                    <button 
                        disabled={selectedRooms.length === 0 || !selectedBookingClient}
      ...
      {activeTab === 'history' && (
        <>
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800">Histórico de Estadias</h1>
            <div className="flex items-center gap-2">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {(() => {
                  ...
                  let completedStays = stays.filter(s => s.status === 'completed');
                  ...
                  completedStays.sort((a, b) => {
                    ...
                    switch (historySortOrder) {
                      ...
                      case 'name-desc':
                        return (clientB?.name || '').localeCompare(clientA?.name || '');
                      default:
                        return 0;
                    }
                  });
                  ...
                  return completedStays.length > 0 ? (
                      ...
                      return (
                        <tr key={stay.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 text-sm text-gray-600">
                            {new Date(stay.checkOutDate).toLocaleDateString('pt-BR')}
                          </td>
                          <td className="px-6 py-4">
                            <div>
                              <p className="font-medium text-gray-800">{client?.name || 'N/A'}</p>
                              <p className="text-xs text-gray-500">ID: {stay.id}</p>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <span className={`text-xs px-2 py-1 rounded-full font-bold ${
                              room?.category === 'Presidente' ? 'bg-yellow-100 text-yellow-800' : 
                              room?.category === 'Luxo' ? 'bg-purple-100 text-purple-800' : 
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600">
                            <div>
                              <p>{new Date(stay.checkInDate).toLocaleDateString('pt-BR')} - {new Date(stay.checkOutDate).toLocaleDateString('pt-BR')}</p>
                              <p className="text-xs text-gray-400">{stay.totalDays} diária(s)</p>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div>
                              ...
                              {extraCharges > 0 && (
                                <p className="text-xs text-orange-600">+ R$ {extraCharges.toFixed(2)} extras</p>
                              )}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                        {historySearchTerm ? 'Nenhuma estadia encontrada com os critérios de busca.' : 'Nenhuma estadia finalizada ainda.'}
                      </td>
                    </tr>
                  );
                })()}
              ...
              <div className="flex gap-2">
                <div className="w-1/3">
                    ...
                    <div className="flex gap-2">
                        ...
                        />
                        <button type="button" onClick={handleCepLookup} className="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700">
                            <Search size={20} />
                        </button>
                    </div>
                </div>
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6 space-y-2">
                 ...
                 <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="border-t border-gray-200 pt-3 flex justify-between items-center mt-2">
                    ...
                    <span className="text-2xl font-bold text-gray-800">R$ {(checkoutData.roomRate + checkoutData.totalExtras).toFixed(2)}</span>
                  </div>
               </div>
               ...
               {checkoutData.client.points > 0 && (
                 <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-5 rounded-xl border-2 border-yellow-200 mb-6">
                   <div className="flex items-center gap-2 mb-3">
                     ...
                     <h3 className="font-bold text-gray-800">Usar Pontos de Fidelidade</h3>
                   </div>
                   
                   <div className="bg-white p-3 rounded-lg mb-3">
                     <div className="flex justify-between items-center mb-2">
                       <span className="text-sm text-gray-600">Pontos disponíveis:</span>
                       <span className="font-bold text-yellow-700">{checkoutData.client.points} pontos</span>
                     </div>
                     <div className="flex justify-between items-center text-xs text-gray-500">
                       <span>Conversão:</span>
                       <span>{loyaltyConfig.pointsPerDiscount} pontos = 1% desconto</span>
                     </div>
                   </div>
                   ...
                   <div className="space-y-3">
                     ...
                     {pointsToUse > 0 && (() => {
                       const discountPercentage = pointsToUse / loyaltyConfig.pointsPerDiscount;
                       const subtotal = checkoutData.roomRate + checkoutData.totalExtras;
                       ...
                       return (
                         <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                           <div className="flex justify-between items-center text-sm mb-1">
                             <span className="text-green-700 font-medium">Desconto ({discountPercentage.toFixed(1)}%):</span>
                             <span className="text-green-700 font-bold">-R$ {discount.toFixed(2)}</span>
                           </div>
                           <div className="flex justify-between items-center">
                             <span className="text-gray-600 text-xs">Novo total:</span>
                             <span className="text-lg font-bold text-green-700">R$ {finalTotal.toFixed(2)}</span>
                           </div>
                         </div>
                       );
                     })()}
